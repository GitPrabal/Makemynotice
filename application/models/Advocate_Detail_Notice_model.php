...
class Advocate_Detail_Notice_model extends CI_Model {
...
public function misconduct_list($id){
    ...
    $this->load->library('session');
    $this->db->select('*');
    $this->db->from('misconduct_notice')->join('advocate_pulled_notice','advocate_pulled_notice.notice_id = misconduct_notice.id','left');
    $this->db->where('misconduct_notice.id',$id);
    $query = $this->db->get();
    $data  = $query->result();
...
public function suspension_notice($id){
...
public function harrashment($id){
